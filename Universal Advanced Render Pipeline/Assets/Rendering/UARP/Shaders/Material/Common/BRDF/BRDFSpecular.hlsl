...
half3 ComplexDirectBRDFSpecular(BRDFData brdfData, VectorsData vData, half3 lightDirectionWS, half3 specular, half anisotropy)
{
    ...
    half specularTerm = DV_Anisotropy(vData, brdfData.perceptualRoughness, anisotropy, lightDir);
    #else
    half specularTerm = brdfData.roughness2 / (d2 * max(half(0.1), LdotH2) * brdfData.normalizationTerm);
    #endif
...
inline half3 DirectBDRF(BRDFData brdfData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS, bool specularHighlightsOff)
{
...
inline half3 ApplyClearCoatBRDF(SurfaceData surfaceData, BRDFData brdfDataCoat, VectorsData vData, half3 brdf, half3 lightDir)
{
        ...
        half NdotV = saturate(dot(clearCoatNormalWS, vData.viewDirectionWS));
    
        half coatFresnel = kDielectricSpec.x + kDielectricSpec.a * Pow4(1.0 - NdotV);
