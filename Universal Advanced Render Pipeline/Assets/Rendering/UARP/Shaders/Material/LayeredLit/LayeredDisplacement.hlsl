#ifndef URPPLUS_LAYERED_DISPLACEMENT_INCLUDED
#define URPPLUS_LAYERED_DISPLACEMENT_INCLUDED

#include "Assets/Rendering/UARP/Shaders/Material/LayeredLit/LayeredSurfaceInput.hlsl"

#if defined(_PIXEL_DISPLACEMENT) && defined(_DEPTHOFFSET_ON)
    #define _DEPTHOFFSET
#endif

real3 GetViewDirectionTangentSpace(real4 tangentWS, real3 normalWS, real3 viewDirWS)
{
    real3 unnormalizedNormalWS = normalWS;
    const half renormFactor = 1.0 / length(unnormalizedNormalWS);

    real crossSign = (tangentWS.w > 0.0 ? 1.0 : -1.0);
    real3 bitang = crossSign * cross(normalWS.xyz, tangentWS.xyz);

    real3 WorldSpaceNormal = renormFactor * normalWS.xyz;
    real3 WorldSpaceTangent = renormFactor * tangentWS.xyz;
    real3 WorldSpaceBiTangent = renormFactor * bitang;

    real3x3 tangentSpaceTransform = real3x3(WorldSpaceTangent, WorldSpaceBiTangent, WorldSpaceNormal);
    real3 viewDirTS = mul(tangentSpaceTransform, viewDirWS);
...
real3 ApplyVertexDisplacementWS(half4 vertexColor, real3 positionOS, real3 normalWS, float2 uv)
{
    ...
    half3 height = half3(0.0, 0.0, 0.0);
        ...
        InitializeTexCoordinates(uv, layerTexCoord);
        height = ComputePerVertexDisplacement(layerTexCoord, vertexColor, 1);
    #endif
    #if defined (_SNOW_DISPLACEMENT) && (_SNOW_HEIGHTMAP)
        UVMapping uvMapping = InitializeUVData(positionWS, normalWS, uv);
        height += ComputePerVertexSnowHeightMap(uvMapping, positionOS.xyz);
    #endif
...
VertexPositionInputs CalculateVertexPositionInputs(half4 vertexColor, real3 positionOS, real3 normalWS, float2 uv)
{
    VertexPositionInputs input;

    input.positionWS = ApplyVertexDisplacementWS(vertexColor, positionOS, normalWS, uv);
